...
function CounterEffect() {
  ...
  const buttonClick = () =>{
    ...
    // useEffect와 차이점 비교테스트
    // 1. 초기에 타이틀이 안바뀌어 있음(왜? 처음 마운트시 실행안됨)
    // 2. 버튼 클릭 시 count 값이 제대로 반영 안됨 (왜? set 함수는 비동기 함수)

  }

  // 실행시점 :HTML DOM 렌더링 직후(즉, 우리가 만든 컴포넌트 화면에 나타난 이후)
  // 비동기함수, 렌더링 마친 직후에 실행되요 useEffcet
  useEffect(()=>{
  ...
  return (
    ...
    <button type="button" onClick={buttonClick}>버튼클릭</button>
    </>
  );
  // (정리)
