...
import axios from "axios";  //중괄호 넣지말고 그냥 import해서 넣어주세요
import { json } from "react-router-dom";

function ApiRequest() {
  // 서버에서 가져온 데이터를 담을 State 만들어요
  const [data, setData] = useState(null);

  // 1. promise / then방식
  const handleRequestById = (id) => {
    axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`)
      .then((response) => {     //앞의 결과가 프로미스 객체가 오기 때문에 then 가능해 
      ...
      return (
        <>
          <div>
            ...
            <button type="button" onClick={() => { handleRequestByIdAsync(10) }}>불러오기</button>
          </div>
          {data && (
            <>
