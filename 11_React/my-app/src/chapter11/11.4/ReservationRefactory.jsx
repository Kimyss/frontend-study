...
// 개선한 코드
function ReservationRefactory() {
  // 만약 여러개의 state가 서로 관련이 있는 데이터라면 객체 형태로 묶어서 관리가능
  ...
  const {breakfast, numberOfGuests, roomType} = inputs;  //. 찍기 말구 구조 분해 할당 통해 값 추출

  // ★이 여러개의 입력제어를 하나로 관리 위의 useState()보시길 -
  ...
  const handleInputChange = (e) =>{
    ...
    // 중요!
    // 리액트 상태에서 객체를 수정해야 할 때에에는,
    // 아래와 같이 기존 상태를 직접 수정해서 set함수에 넣으면 안됨
    inputs[name] = inputValue;
    // setInputs(inputs);
    // => inputs 가리키는 객체의 내부데이터만 바뀐것이지 주소값(참조값)은 변하지 않았기 때문

    // 그 대신 새로운 객체를 만들어서 변화를 주고 이를 상태로 사용해야 함
    // 이러한 작없을 "불변성을 지킨다." 라고 부름(기존 객체는 변경하지X)
    ...
    // 방법2 : 코드 단축 버전 (ES6차 문법 이용해서 ~)
    setInputs({
      ...inputs,
      [name]: inputValue
    });    //바로 추출하고 기존객체 복사본 동시에 만들어 ES6차 오브젝트리터럴참조

    // 결론 : 리액트 상태에서 객체를 업데이트 할때에는 기존 객체를 직접 수정하면 안되고,
    // 새로운 객체(기존객체의 복사본)를 만들고 그 객체에 변화를 주고 마지막으로 set함수에 넣어줘야함
  ...
  // 여러 입력 양식을 제어해야할 때, 각 엘리먼트에 name속성을 추가하고
  // event.target.name 값을 통해 어떤 입력 양식인지 구분할 수 있게 해줌
  // 그리고 useState에서는 기본타입의 값이 아니라 객체 형태의 상태를 관리
...
export default ReservationRefactory;